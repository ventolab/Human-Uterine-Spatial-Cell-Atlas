"""
Map spatial axis values from Xenium onto epithelial scRNA-seq reference data using
iss_patcher.
...
─────────────
concat / unique:
  axis_mapping_outputs/{mode}_epithelium_{stage}_k{k}/
    iss_patched.h5ad
    axis.csv

ensemble:
  axis_mapping_outputs/ensemble_epithelium_{stage}_k{k}/
    {sample}/
      iss_patched.h5ad
...
# for lineage filtering via TACCO celltype prefix (Epi_*)
...
def parse_args() -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Map continuous axis values from spatial to epithelial scRNA-seq data."
    )
    parser.add_argument(
        "--mode",
...
def main() -> None:
    ...
    print(f"\n{'='*70}", flush=True)
    print("Spatial-to-scRNA Axis Mapping Pipeline  [epithelium]", flush=True)
    ...
    for stage in stages:
        print(f"\n{'─'*60}", flush=True)
        print(f"Stage: {stage}  |  Mode: {args.mode}", flush=True)
