# Assign TACCO-predicted axis bins and create pseudobulk profiles for epithelial
...
adata_epithelium = sc.read_h5ad(
    "/nfs/team292/projects/PanTissue/results/temp/02_annotation/"
    "annotated_postnatal_epithelial_endometrium.h5ad"
)
adata_epithelium.obs["sample"] = adata_epithelium.obs["Donor_id"].astype(str)
...
adata_epithelium.obs = adata_epithelium.obs.join(tacco_df[["axis_bin"]], how="left")

# Drop cells without a TACCO prediction
n_before = adata_epithelium.shape[0]
adata_epithelium = adata_epithelium[adata_epithelium.obs["axis_bin"].notna()].copy()
print(f"Dropped {n_before - adata_epithelium.shape[0]:,} cells without TACCO predictions; "
      f"{adata_epithelium.shape[0]:,} cells retained")

# Enforce ordered categorical so groupby respects anatomical direction
adata_epithelium.obs["axis_bin"] = pd.Categorical(
    adata_epithelium.obs["axis_bin"], categories=BIN_ORDER, ordered=True
)

# ── Pseudobulk per sample × axis_bin ─────────────────────────────────────────

MIN_CELLS_PER_BIN  = 20
MIN_BINS_PER_DONOR = 2

groups = adata_epithelium.obs.groupby(["sample", "axis_bin"], observed=True)
pb_counts, pb_meta, col_names = [], [], []

for (sid, binn), group_df in groups:
    raw    = adata_epithelium[group_df.index].X
...
gene_names = adata_epithelium.var_names
pb_df      = pd.DataFrame(pb_matrix, index=gene_names, columns=col_names)
pb_df.to_csv("epithelial_pb_matrix_tacco_ensemble.tsv", sep="\t")
pb_meta.to_csv("epithelial_pb_meta_tacco_ensemble.tsv", sep="\t", index=False)
...
donor_qc_out.to_csv("epithelial_pb_donor_qc_tacco_ensemble.tsv", sep="\t", index=False)

print("\nOutputs written:")
print("  epithelial_pb_matrix_tacco_ensemble.tsv")
print("  epithelial_pb_meta_tacco_ensemble.tsv  (includes donor_n_bins column)")
print("  epithelial_pb_donor_qc_tacco_ensemble.tsv")
