"""
Map spatial tissue-zone annotation onto mesenchymal scRNA-seq cells using TACCO,
running one spatial sample at a time and ensembling predictions by averaging
scores across samples before taking argmax.

Reference = spatial (Xenium), filtered to Mesen cells per sample
...
def assign_axis_bins(adata: ad.AnnData) -> ad.AnnData:
    """Add axis_bin column to adata.obs in-place and drop NaN rows."""
...
def ensemble_and_save(
    ...
    # Stack and average — align columns so all bins are present in each df
    all_cols = BIN_CATEGORIES
    aligned  = [df.reindex(columns=all_cols, fill_value=0.0) for df in scores_list]
...
def main() -> None:
    print(f"\n{'='*70}", flush=True)
    print("TACCO  ensemble  spatial annotation  →  mesenchymal scRNA-seq", flush=True)
    ...
    for stage, slide_list in SPATIAL_BY_STAGE.items():
        ...
        for spatial_path, pred_path in slide_list:
            ...
            # Check cache before loading the heavy spatial file
            ...
            if cached.exists():
                ...
                reference = load_spatial_mesen_reference(spatial_path, pred_path)
